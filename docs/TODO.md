...
当前合同见 [便携相册与第一阶段特征索引设计](index-design.md)。[便携相册实施计划](portable-album-plan.md) 保留 schema 8 历史里程碑，不是 schema 10 六组件的实时进度；[早期实施计划](ingestion-index-management-plan.md) 的多相册、旧 CLI 和迁移设计亦已被取代。以下区分实现范围、历史回归及待核实验收，不发布私人路径、照片或报告数据。
...
- **新格式边界。** schema 10、`application_id=0x53414C42`，37 张注册表：32 张普通表、1 张 external-content FTS5 虚拟表、4 张显式登记的影子表（不计 SQLite 内部 `sqlite_sequence`）。完整清单见现行设计；原六张 `image_embedding_*` 不变，新公共 feature 配置/结果/manifest/依赖/任务及类型明细独立存储。`virtual_folders`、`virtual_folder_photos` 保留。没有旧多相册或空 `technical_*` 表。旧 v1–v9 数据库原样拒绝，不迁移、清空或覆盖；真实旧库及备份保持不动。管理快照仍为 `album-snapshot-v2`。
...
- 六组件必须显式 `--component`；未传仍只做 `image_embedding`，基础三能力和 ingestion 邀请不变。setup/`register-profile` 不设默认，各组件独立 configure。修改有效参数/资产/recipe/dependency 产生新 profile；`input_fingerprint` 保存输入/精确依赖身份而非路径，历史不覆盖。CLI 见 [index](../photography/references/index.md#feature-cli)。
...
- [ ] **技术参数：对焦/曝光等仍为后续 index 工作，不新增第四能力。** 不把六组件或 embedding ready 当作技术质量已完成；明确输入范围、单位、算法/profile 和独立结果/授权，不预建空表/空结果。缺失数据是未知；缩略图模糊不等于原图对焦失败，主观描述不是测量。
