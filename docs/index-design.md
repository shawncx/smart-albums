...
- New albums use `PRAGMA application_id = 0x53414C42`, `PRAGMA user_version = 12` and rollback journaling rather than default WAL. Commands close connections on completion.
...
## 3. Schema 12: 40 registered tables
...
### Index coverage and legacy plain-search snapshots
...
Success uses strict `photo-review-v2` JSON with only `results` at the top level, manifest-ordered opaque IDs, `review_status`, `description`, `dimensions`, `strengths`, structured `improvements`, and nonempty `limitations`. Each dimension accepts a 0–10 score in 0.5 steps or null plus a reason; status must agree with numeric/null counts. The application computes the equal-weight mean with decimal half-up rounding to two decimal places only for six numeric scores. Unknown scores and totals persist as SQL NULL. The parser accepts bare JSON or one complete triple-backtick JSON/unlabeled code block, then applies the same strict payload validation; surrounding prose, multiple/nested blocks and malformed JSON are rejected. Failed validation stores content-free envelope/size/hash diagnostics in the existing error details, preserved by retry history; no schema change or raw-response storage is introduced. Structural validation does not establish visual grounding, correct language or complete preview-limit explanations. Historical v1 profiles/payloads remain exact; new reviews use v2 and cannot reuse v1 cache. `review upgrade --output <new.sqlite>` publishes a validated schema 12 copy and preserves the schema 11 source. No migration occurs on open.

Reuse keys bind saved content/preview identity and output-affecting config, not paths or batch companions. Force appends immutable history. New plans freeze `max_concurrency` (default 5, positive integer) alongside `batch_size` (default 4); legacy plans without the concurrency field remain serial. Provider calls run in a bounded thread pool with independent clients/sessions and a synchronized shared login check; SQLite stays on the caller thread. Network calls stay outside SQL transactions; revalidate identities before transmission and atomic whole-batch commit. An observed failure or caller interrupt stops new submissions, drains already submitted requests and saves valid complete batches before releasing the album lock or exposing a retry digest. Batch ordinals and result identities remain frozen despite out-of-order completion. No automatic retries/repair calls/fallback. All-cached/completed work sends nothing; uncertain attempts carry duplicate-charge risk. Current/stale result/history reads use saved identities, without originals or AI. This does not change existing local ingestion/index/search behavior.
