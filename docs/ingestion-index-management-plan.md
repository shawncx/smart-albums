...
> 历史计划：下文的旧多相册、迁移、CLI 兼容及验收记录保留原意，不是现行使用合同。当前见 [现行设计](index-design.md)：一个 SQLite 文件一个相册，schema 9、13 张表，`album-snapshot-v2`；v1–v8 原样拒绝且不迁移。management 现已包含静态 virtual folder 手动管理、范围浏览/搜索及经确认的一次性日期整理，不新增能力或自动归类。
