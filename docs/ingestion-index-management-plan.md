...
状态：方案已确认，尚未实施。本文保存已确认的实现计划，不代表已执行代码改造或模型推理。
...
原有 OpenAI/Codex 分析、相册写操作和 `search-add` 保留为兼容接口，
不作为新 Skill 的第四类能力，也不从新 management 只读流程触发。
...
- 保留旧 JSON 字段供兼容，但不将旧 `analysis_summary` 解释成 index 覆盖率。
...
新增独立图片索引存储，旧表及所有旧记录原样保留，不把旧描述向量迁入新空间。
...
- v5 和更早支持版本先做一致性备份；新增 v6 表并验证后才设置版本号。
- 初次空库直接创建 v6；保留旧 v1/v2 缩略图迁移链，不重复读取原图。
- 检查 integrity、外键、旧表行数/内容摘要、相册关系、缩略图 BLOB、
  旧分析/向量及旧工作流记录；迁移失败完整回滚。
...
| `storage.py`、`sqlite_storage.py` | 新协议、v6 迁移及新存储接线 |
...
- 更新 `references/albums.md`、`references/search.md`，旧分析 references 加兼容说明，
  删除失效命令和旧模型安装指导，不让多个文档给出冲突流程。
...
- 文档中的 SQL/JSON 若保留为示例，必须由实际 v6 合同和合成 fixture 校验；
  不发布另一份与迁移代码脱节的可执行草案。
...
3. v1–v5 到 v6 的升级、备份、旧行/BLOB 保留、失败回滚和外键约束。
...
按修改范围合并同 runner 的针对性测试。v6 修改会影响多个现有测试中的
...
2. **实现 v6 存储**：迁移、历史结果、任务状态、配置和占用；依赖 1。
