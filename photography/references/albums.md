...
multiple-albums-per-database interface. There are exactly four Skill capabilities;
...
separate capability. `management folders` supports empty custom folders and stable
...
`album-snapshot-v2` with historical album/folder scope. Old v1–v10 databases are rejected unchanged,
...
The three ordinary `ai_review_results` / `ai_review_runs` / `ai_review_batches` tables also travel with backups. Optional [review](review.md) needs explicit whole-task approval before Copilot contact and fresh approval per retry; it adds no automatic folder writes or review-aware search in v1.
