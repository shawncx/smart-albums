...
The snapshot's `query_encoding` freezes `strategy`, original `query`, English `visual_query`, actual `prompts` and `weights`. `show-results` and search-selected folder-add provenance preserve it. Show the user the actual encoded text, not only their original request. This query-only preparation leaves the image profile, checkpoint, 768-dimensional vectors and supported schema 11/12 unchanged; no image reindexing is required. The recipe itself does not establish measured retrieval quality.
