...
### Output reliability and private diagnostics

The pinned SDK's ordinary review request does not enable server-enforced JSON Schema output. The schema is included in the prompt as text; strict parsing and domain validation protect saved results, not the probability of a valid provider response. The prompt also supplies a complete syntax-only example, multiline formatting and explicit container-closing checks. The example's null scores describe an unreadable attachment, not score defaults for real photos. These instructions do not relax the rubric or authorize repairing malformed responses.

Syntax failures include content-free diagnostics: `response_format`, UTF-8 `response_bytes`, `response_chars`, SHA-256, a normalized `json_error_code`, `json_error_character`, `json_error_offset`, `json_body_chars`, `json_remaining_chars`, `json_error_at_end`, and line/column. Offsets and lengths in the `json_*` fields count characters in the parsed body after removing an allowed single code-block wrapper, not UTF-8 bytes or the wrapper. Error kinds and character categories use fixed labels, never provider text or arbitrary parser messages. A near-end error is evidence about the received text, not proof of provider truncation.

Failed batch error details also preserve `provider_response`: complete-message event counts, observed idle completion, reported chunk counts when available, normalized provider finish reasons and reported output-token limits. These diagnostics are separate from the schema's fixed usage metadata and require no album migration. Missing finish reasons are `unavailable`, unrecognized values become `other`, and differing reported reasons become `mixed`; arbitrary provider strings are not retained. `response_output_limit_calls` counts only explicitly reported `length`/`max_tokens` reasons and must be read alongside `response_finish_reason_calls` and `response_usage_calls`, not treated as proof of completeness when telemetry is missing. An observed idle event is not a guarantee of valid or untruncated JSON.

Keep failed response bodies, text excerpts and image payloads out of logs and saved diagnostics. A single complete JSON or unlabeled code block is accepted; missing braces, extra prose, duplicate keys, non-finite numbers, field/ID mismatches and invalid scores still fail. Every failed request retains available usage without saving a partial review or triggering automatic repair, fallback or retry.
...
The expanded score-justification instructions were added after those live trials and were retained in the Terra format-stability trial below. Format success does not establish better visual judgments or score explanations. Earlier saved reviews retain their original prompt and reasons.

**Authorized Terra format-stability validation (2026-09-10):** a two-photo diagnostic run with the earlier prompt reproduced one syntax failure: `expected_comma` at an `array_end`, with two characters remaining. A synthetic missing-result-brace regression reproduces this signature; no invalid response body was retained or repaired. After adding the complete syntax example and explicit closing checks, the same fixed ten-photo cohort passed 40/40 fresh single-image requests at maximum concurrency 5. This includes a candidate round and **three consecutive final-code rounds, 30/30**, with matching source-file fingerprints, prompt hash and stored preview identities. No result-cache reuse, provider retry, JSON repair, partial/null-score substitute or model change was used. Reopening the album verified 30 distinct final-round results and all 180 numeric dimension scores.

The improvement campaign made 42 requests total, including the two original-prompt diagnostic requests: 374,962 reported input tokens, 49,182 output tokens and 271.078 active seconds, including the diagnostic failure. These totals exclude the separate earlier 12-request run. Active execution excludes planning, confirmation waits and report generation; concurrent batch times are not summed. All 121 related local tests passed using the existing compatible test dependencies. This is observed format stability on a repeated fixed cohort, not a guarantee for new photos, future provider versions or photographic assessment quality.
