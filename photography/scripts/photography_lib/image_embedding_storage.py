"""Versioned image-embedding results, plans, progress and concurrency claims."""
...
def profile_identity(profile):
    if not isinstance(profile, dict) or not profile:
        raise PhotographyError("INDEX_PROFILE_INVALID", "An image-embedding profile must be a nonempty object.")
    semantic_fields = {
    ...
    if any(profile.get(key) != value for key, value in semantic_fields.items()):
        raise PhotographyError("INDEX_PROFILE_INVALID",
            "Image-embedding profiles must identify whole-image semantic vectors from stored thumbnails.")
    try:
        ...
        raise PhotographyError("INDEX_PROFILE_INVALID", "The image-embedding profile must contain finite JSON values.") from exc
    return fingerprint(profile), encoded


IMAGE_EMBEDDING_TABLES = (
    "image_embedding_profiles", "image_embedding_results", "image_embedding_runs",
    "image_embedding_items", "image_embedding_claims", "image_embedding_settings",
)
...
IMAGE_EMBEDDING_SCHEMA = (
    ...
    """CREATE TABLE IF NOT EXISTS image_embedding_results (
        result_id TEXT PRIMARY KEY,
        photo_id TEXT NOT NULL REFERENCES photos(photo_id),
        profile_id TEXT NOT NULL REFERENCES image_embedding_profiles(profile_id),
    ...
    "CREATE INDEX IF NOT EXISTS image_embedding_results_profile ON image_embedding_results(profile_id,photo_id)",
    """CREATE TABLE IF NOT EXISTS image_embedding_runs (
    ...
    """CREATE TABLE IF NOT EXISTS image_embedding_items (
        ...
        FOREIGN KEY(run_id,profile_id) REFERENCES image_embedding_runs(run_id,profile_id),
    ...
    """CREATE TABLE IF NOT EXISTS image_embedding_claims (
        ...
        FOREIGN KEY(run_id,photo_id,profile_id,content_version,thumbnail_profile,input_image_hash)
            REFERENCES image_embedding_items(run_id,photo_id,profile_id,content_version,thumbnail_profile,input_image_hash))""",
    """CREATE TABLE IF NOT EXISTS image_embedding_settings (
        setting_key TEXT PRIMARY KEY, profile_id TEXT NOT NULL REFERENCES image_embedding_profiles(profile_id))""",
    """CREATE TRIGGER IF NOT EXISTS image_embedding_profile_immutable
        BEFORE UPDATE ON image_embedding_profiles
        BEGIN SELECT RAISE(ABORT,'Image-embedding profiles are immutable'); END""",
    """CREATE TRIGGER IF NOT EXISTS image_embedding_plan_immutable
        BEFORE UPDATE OF run_id,profile_id,digest,plan_json,created_at ON image_embedding_runs
        BEGIN SELECT RAISE(ABORT,'Image-embedding plans are immutable'); END""",
    """CREATE TRIGGER IF NOT EXISTS image_embedding_snapshot_immutable
        BEFORE UPDATE OF run_id,photo_id,profile_id,content_version,thumbnail_profile,input_image_hash,
            action,snapshot_json ON image_embedding_items
        BEGIN SELECT RAISE(ABORT,'Image-embedding input snapshots are immutable'); END""",
    """CREATE TRIGGER IF NOT EXISTS image_embedding_result_identity_immutable
        BEFORE UPDATE OF result_id,photo_id,profile_id,content_version,thumbnail_profile,input_image_hash
            ON image_embedding_results
        BEGIN SELECT RAISE(ABORT,'Image-embedding result identities are immutable'); END""",
)


class ImageEmbeddingStorage:
    def put_embedding_profile(self, profile):
        profile_id, encoded = profile_identity(profile)
        with self.savepoint():
            self.db.execute("""INSERT INTO image_embedding_profiles VALUES (?,?,?)
                ON CONFLICT(profile_id) DO NOTHING""", (profile_id, encoded, timestamp()))
            if self.embedding_profile(profile_id) != profile:
    ...
    def embedding_profile(self, profile_id):
        row = self.db.execute(
            "SELECT profile_json FROM image_embedding_profiles WHERE profile_id=?", (profile_id,)).fetchone()
        if row is None:
            raise PhotographyError("INDEX_PROFILE_NOT_FOUND", "Image-embedding profile does not exist.")
        try:
    ...
    def embedding_profiles(self):
        return [{"profile_id": row["profile_id"], "profile": self.embedding_profile(row["profile_id"]),
                 "created_at": row["created_at"]}
                for row in self.db.execute(
                    "SELECT profile_id,created_at FROM image_embedding_profiles ORDER BY created_at,profile_id")]

    def default_embedding_profile(self):
        row = self.db.execute(
            "SELECT profile_id FROM image_embedding_settings WHERE setting_key='default_profile'").fetchone()
        return row[0] if row else None

    def set_default_embedding_profile(self, profile_id):
        with self.savepoint():
            self.embedding_profile(profile_id)
            self.db.execute("""INSERT INTO image_embedding_settings VALUES ('default_profile',?)
                ON CONFLICT(setting_key) DO UPDATE SET profile_id=excluded.profile_id""", (profile_id,))

    @property
    def embedding_lock_path(self):
        path = self.db.execute("PRAGMA database_list").fetchone()[2]
        return Path(str(Path(path).resolve()) + ".image-embedding.lock")

    def _embedding_result(self, snapshot, profile_id):
        row = self.db.execute("""SELECT * FROM image_embedding_results WHERE
    ...
    def _has_embedding_results(self, photo_id, profile_id):
        return self.db.execute(
            "SELECT 1 FROM image_embedding_results WHERE photo_id=? AND profile_id=? LIMIT 1",
            (photo_id, profile_id)).fetchone() is not None

    def _put_embedding_result(self, snapshot, profile_id, blob, vector_hash, dimensions):
        old = self._embedding_result(snapshot, profile_id)
        result_id = old["result_id"] if old else "emb_" + uuid4().hex
        self.db.execute("""INSERT INTO image_embedding_results
    ...
    def _create_embedding_run(self, plan):
        self.db.execute("""INSERT INTO image_embedding_runs
        ...
        for snapshot in plan["snapshots"]:
            self.db.execute("""INSERT INTO image_embedding_items
    ...
    def _embedding_run(self, run_id):
        row = self.db.execute("SELECT * FROM image_embedding_runs WHERE run_id=?", (run_id,)).fetchone()
        if row is None:
            raise PhotographyError("INDEX_RUN_NOT_FOUND", "Image-embedding run does not exist.")
        result = dict(row)
        try:
            result["plan"] = json.loads(result.pop("plan_json"))
        except (TypeError, ValueError) as exc:
            raise PhotographyError("INDEX_PLAN_INVALID", "Stored image-embedding plan is invalid.") from exc
        return result

    def _embedding_items(self, run_id):
        items = []
        for row in self.db.execute("SELECT * FROM image_embedding_items WHERE run_id=? ORDER BY photo_id", (run_id,)):
            item = dict(row)
            try:
                ...
                raise PhotographyError("INDEX_PLAN_INVALID", "Stored image-embedding item is invalid.") from exc
            items.append(item)
        return items

    def _update_embedding_run(self, run_id, *, status=None, confirmed_digest=None, model_call=False):
        ...
        self.db.execute(f"UPDATE image_embedding_runs SET {','.join(updates)} WHERE run_id=?", (*values, run_id))

    def _update_embedding_item(self, run_id, item):
        self.db.execute("""UPDATE image_embedding_items SET
    ...
    def _claim_embedding_input(self, run_id, snapshot, profile_id):
        row = self.db.execute("""SELECT run_id FROM image_embedding_claims WHERE
            photo_id=? AND profile_id=? AND content_version=? AND thumbnail_profile=? AND input_image_hash=?""",
            (snapshot["photo_id"], profile_id, snapshot["content_version"],
             snapshot["thumbnail_profile"], snapshot["input_image_hash"])).fetchone()
        if row:
            raise PhotographyError("INDEX_IN_PROGRESS", "This stored image input is already claimed.")
        self.db.execute("INSERT INTO image_embedding_claims VALUES (?,?,?,?,?,?,?)",
            (snapshot["photo_id"], profile_id, snapshot["content_version"],
             snapshot["thumbnail_profile"], snapshot["input_image_hash"], run_id, timestamp()))

    def _release_embedding_input(self, run_id, photo_id):
        self.db.execute("DELETE FROM image_embedding_claims WHERE run_id=? AND photo_id=?", (run_id, photo_id))

    def _recover_embedding_claims(self, run_id):
        # Only this explicitly stopped run is recoverable, under the database-wide OS lock.
        for item in self._embedding_items(run_id):
            ...
            self._update_embedding_item(run_id, item)
        self.db.execute("DELETE FROM image_embedding_claims WHERE run_id=?", (run_id,))
